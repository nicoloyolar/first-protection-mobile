...
Última actualización: 2026-08-19 (Pista C ítem 2 resuelto: cuenta de facturación Blaze vinculada, `deviceApi` desplegado de verdad en producción, primer usuario admin sembrado — y, al probarlo por primera vez de punta a punta, se encontró y corrigió un bug crítico: los comandos reales creados por la app nunca le llegaban al dispositivo. Ver detalle abajo. Pista A y B completas salvo lo pausado; sigue pendiente la decisión de política de corte de corriente).

### Actualización 2026-08-19 — Deploy Real De Cloud Functions

- Cuenta de facturación Blaze vinculada al proyecto `first-protection` (el bloqueo de la sección "Para Retomar" del 12-ago). `npm install` corrido en `functions/` (`node_modules` no estaba instalado).
- **Hallazgo antes de deployar**: no existía `DEVICE_SIM_SECRET` configurado en ningún lado. Sin él, producción habría caído en el fallback del código (`DEFAULT_DEVICE_SECRET = process.env.DEVICE_SIM_SECRET || "dev-secret"`) — un secreto público y predecible para firmar HMAC, justo la superficie más sensible (comandos hacia el actuador). Se generó un secreto de 256 bits y se cargó en Secret Manager vía `firebase functions:secrets:set DEVICE_SIM_SECRET`.
- **Bug encontrado y corregido de paso**: `exports.deviceApi` (`functions/index.js`) no declaraba `secrets: ["DEVICE_SIM_SECRET"]` en sus opciones — en Cloud Functions 2ª gen, un secreto guardado en Secret Manager no llega como variable de entorno a menos que la función lo declare explícito. Sin este fix, el secreto recién creado nunca le habría llegado a la función igual.
- **Hallazgo tipo "documentación vs. código real" otra vez, pero al revés**: al deployar, Firebase reportó una función `enviarAlerta` (callable, 2ª gen, nodejs22) ya existente en producción, **que no está en ningún commit de este repo** y que la app Flutter no llama (sin `cloud_functions`/`httpsCallable` en `lib/`) ni tiene logs de ejecución. Muy probablemente un experimento de notificaciones (brecha 🟠 de Pista A, "Sin notificaciones") deployado directo desde otra máquina o la consola, nunca commiteado. Se confirmó con el usuario y se borró (`firebase functions:delete enviarAlerta --region us-central1`) para poder continuar el deploy.
- **Deploy exitoso**: `deviceApi` corriendo en `https://us-central1-first-protection.cloudfunctions.net/deviceApi`. Verificado con una petición GET no destructiva (404 controlado, sin escribir en la base).
- Aviso de la CLI a atender más adelante (no bloqueante): runtime Node.js 20 deprecado desde 2026-04-30, se decomisiona 2026-10-30 — hay que migrar a una versión soportada antes de esa fecha. `firebase-functions` también quedó marcado como desactualizado.
- Cambio de código (`secrets: [...]` en `functions/index.js`) queda sin commitear todavía.

### ⚠️ Bug Crítico Encontrado Y Corregido (2026-08-19): Comandos Reales Nunca Le Llegaban Al Dispositivo

El hallazgo más importante del día, encontrado al hacer el primer smoke test end-to-end contra `deviceApi` ya desplegado (nunca antes probado así):

- **La causa**: `nextCommand`/`ackCommand` en `functions/index.js` solo leían/escribían un `Map` en memoria del proceso (`memoryStore`), poblado únicamente cuando un comando se crea a través del propio endpoint HTTP `POST /commands` de ese mismo archivo. Pero la app y el panel **nunca llaman ese endpoint** — escriben el comando directo en `device_commands/{deviceId}` vía el SDK de Firebase (`database_service.dart:crearComandoDispositivo`, `.push()`). Resultado: en producción, cuando la app/panel creaba un comando real, la función que atiende al dispositivo (`GET /commands/next`) nunca se enteraba — su memoria seguía vacía. Y aunque coincidiera por casualidad, Cloud Functions 2ª gen escala con instancias efímeras sin memoria compartida (`minInstances: 0`), así que tampoco habría funcionado de forma confiable en la nube real.
- **Por qué nadie lo había detectado**: los 22 tests automatizados y las pruebas manuales anteriores contra el emulador siempre crearon el comando a través del mismo endpoint HTTP (mismo proceso, misma memoria) antes de pedirlo — nunca se probó "la app escribe directo en RTDB → el dispositivo pregunta por HTTP", que es el flujo real. El primer smoke test de hoy contra la función ya desplegada, con un comando escrito directo en la base (igual que lo hace la app), lo expuso de inmediato: el simulador nunca recibió el comando.
- **Bug adicional encontrado de paso en el mismo código**: `ackCommand`, cuando no encontraba el comando en memoria, escribía el objeto `ack` completo pisando el nodo entero en `device_commands/{deviceId}/{commandId}` — perdiendo `type`/`target`/`requestedBy`/etc. del comando original.
- **Fix aplicado**: `nextCommand`/`ackCommand`/el contador `pendingCommands` ahora leen y escriben contra Realtime Database directamente cuando hay DB real disponible (nueva función pura `pickPendingCommand`, exportada y testeada), usando `memoryStore` solo como fallback para desarrollo local/tests sin Firebase (comportamiento sin cambios ahí — los 22 tests originales siguen pasando tal cual). El ACK ahora hace un `update()` dirigido solo a los campos del ACK, nunca un overwrite completo, así que no puede volver a perder los campos originales del comando.
- **4 tests nuevos** para `pickPendingCommand` (sin comandos, comando encontrado con la forma real de datos sin campo `commandId` embebido, comandos expirados/no-pendientes ignorados, orden FIFO con varios pendientes) — total 26 tests, todos verdes.
- **Verificado contra producción real**: se repitió el mismo escenario que expuso el bug (comando escrito directo en `device_commands/` simulando la app, sin campo `commandId` en el value — igual que `DeviceCommand.toMap()` en Dart) contra la función ya redeployada — el simulador esta vez sí recibió el comando, lo ejecutó, y `dispositivos/{id}/humo` se actualizó a `true` solo después del ACK. Datos de prueba limpiados después.
- Cambio también sin commitear todavía (junto con el fix de `secrets:[...]`).
...
| 🟢 **Resuelto (2026-08-19)** | Decisión tomada: Cloud Functions (2ª gen), no servicio propio. Deployado de verdad en producción. | `.firebaserc` (proyecto `first-protection`), `functions/index.js:onRequestV2`, `deviceApi` corriendo en `https://us-central1-first-protection.cloudfunctions.net/deviceApi`. | Se eligió Cloud Functions porque el código ya estaba escrito pensando en eso (`exports.deviceApi`) y el proyecto ya está 100% en Firebase (Auth + RTDB) — es el camino de menor fricción para un prototipo de bajo volumen. `minInstances: 0` (sin costo fijo, acepta cold start ocasional). |
...
| 🟠 | Rotación de `deviceSecret` sin implementar. Hoy además no hay secreto por dispositivo: todos comparten un único `DEVICE_SIM_SECRET`. | `functions/index.js:DEFAULT_DEVICE_SECRET`. | Requiere primero un flujo de aprovisionamiento de dispositivo (generar y guardar un secreto por `deviceId`) que hoy no existe — no es solo agregar rotación, es una brecha más grande que la nota original sugería. **Actualización 2026-08-19**: al menos ya no es el fallback débil `"dev-secret"` — se generó un secreto real de 256 bits y quedó en Secret Manager (`firebase functions:secrets:set DEVICE_SIM_SECRET`, declarado en `secrets:[...]` de `deviceApi`), pero sigue siendo un único secreto compartido, no por dispositivo. |
| 🟢 **Resuelto (2026-08-19)** | `DEVICE_API_SKIP_SIGNATURE` existe como variable para saltar la firma HMAC — riesgo si queda activa fuera de desarrollo. | `Guía de desarrollo y entornos` (PDF), sección "Variables y secretos". | Confirmado en el deploy real del 2026-08-19: no se configuró (ni se configuró ninguna otra variable) más allá del secreto `DEVICE_SIM_SECRET`, así que en producción `process.env.DEVICE_API_SKIP_SIGNATURE` es `undefined` y la firma HMAC queda obligatoria. No hay un chequeo automático que lo bloquee si alguien la agrega después — sigue siendo una responsabilidad manual. |
...
⚠️ **Corrección 2026-08-19**: lo anterior describía el diseño correctamente, pero el punto 4 (backend usa el ACK del simulador para actualizar el actuador real) **nunca se había probado con el backend desplegado de verdad hasta hoy** — y al probarlo se encontró que no funcionaba: la función servía los comandos desde memoria del proceso, no desde la base real donde la app los escribe. Ver "⚠️ Bug Crítico Encontrado Y Corregido (2026-08-19)" más abajo para el detalle completo y la corrección ya verificada contra producción.
...
2. ✅ ~~Decidido 2026-08-12: Cloud Functions, código listo y verificado contra el emulador~~ — **deployado de verdad en producción el 2026-08-19** tras vincular la cuenta de facturación Blaze. `deviceApi` corriendo en `https://us-central1-first-protection.cloudfunctions.net/deviceApi` (ver "Actualización 2026-08-19" arriba para el detalle: secreto HMAC generado y declarado en `secrets:[...]`, función huérfana `enviarAlerta` encontrada y borrada).
...
- El backend ya está desplegado de verdad en producción (2026-08-19), no solo verificado en el emulador — la API de dispositivo (`deviceApi`) está corriendo en Cloud Functions con firma HMAC obligatoria.
- Antes de cualquier demo con datos reales o usuarios externos, quedan 2 tareas: restricción de API keys de Google Maps (seguridad, no negociable) y sembrar el primer usuario admin en la base de datos (operativo, sin esto nadie entra al panel). Las reglas de Realtime Database y dejar de escribir actuadores directo desde la UI ya se resolvieron el 2026-08-12.
- Detalle no técnico interesante: al deployar apareció una función (`enviarAlerta`) que ya existía en producción de una sesión anterior fuera de este repo, sin código versionado — se eliminó porque no se usa en ningún lado hoy. Vale la pena preguntar de dónde salió, por si hay trabajo de notificaciones ya empezado que convenga recuperar en vez de rehacer.
...
- ~~**Acción inmediata pendiente**: vincular cuenta de facturación (plan Blaze)~~ — resuelto 2026-08-19, ver "Actualización 2026-08-19 — Deploy Real De Cloud Functions" arriba.
- ~~**Acción inmediata pendiente**: sembrar el primer usuario con `role: admin`~~ — resuelto 2026-08-19. Se confirmó primero que las reglas ya estaban desplegadas en producción (lectura sin auth vía REST directo a la RTDB devuelve `401 Permission denied`). Se creó la cuenta `admin@first-protection.cl` en Firebase Auth (vía Identity Toolkit REST, password generado y entregado solo en el chat, no queda en ningún archivo) y se sembró `usuarios/{uid}` con `role: admin` vía `firebase database:set` (la CLI tiene acceso administrativo y por eso puede escribir ese nodo aunque las reglas normales no lo permitirían todavía a nadie). Verificado leyendo el nodo de vuelta. Pendiente: cambiar el password desde el panel al primer login y guardar credenciales en el gestor de contraseñas del equipo.
- Commitear los cambios de código pendientes en `functions/index.js` — se hicieron en el filesystem y ya se re-deployaron a producción, pero no se ha hecho `git commit` todavía: (1) declaración de `secrets: ["DEVICE_SIM_SECRET"]` en `deviceApi`, (2) fix del bug crítico de comandos (`pickPendingCommand`, `nextCommand`/`ackCommand`/`countPendingCommands` leyendo de RTDB real en vez de solo memoria) + 4 tests nuevos en `functions/test/device-api.test.js`, ver "⚠️ Bug Crítico Encontrado Y Corregido (2026-08-19)" arriba.
